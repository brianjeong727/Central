...
5. **Doc edits** — exact text for any CLAUDE.md / permissions.md / MINISTRY_CONTEXT.md change (per the core's doc-gate table), and a note of any inbox lesson files auto-written (`tasks/lessons/inbox/*.md`). CLAUDE.md and permissions.md are ask-then-write: propose the text, wait for approval in that task, THEN write them yourself — so say plainly which you have written and which are still awaiting a yes. MINISTRY_CONTEXT.md is always proposal-only.
