...
| Back-row ("← Section") | ~~`PocketBackRow`~~ — **deleted 2026-09-18 (K7)**; a drilled screen's chrome row is `PocketChrome`/`PocketHubChrome`/`SubpageShell`, never an in-body back row | retired |
...
- **Row** — two grammars, one primitive (`PocketRow`).
  ...
  - **`trailing` — an optional per-row action.** A row may carry ONE trailing control, and in practice that control is a ⋯ opening the shared `ActionMenu` (Convention #20) for the actions that belong to that row's object — delete, and whatever joins it later. It renders as a SIBLING of the row's button, never inside it: a button nested in a button is invalid, and more practically the action must not share the row's tap target or a thumb aiming at the menu opens the screen instead. Give it a 44px width of its OWN and expand the target vertically with the `.tap-expand-y` pseudo-element (`app/globals.css`) — never a leftward overlay expander reaching back across the row, and never a child `<span>` expander, which leaves the host a non-leaf and makes the screen sweep measure the wrong element (Convention #27). The row divider belongs to the whole row once something sits beside the button, or it stops short under the action. The chevron STAYS: it says the row drills in, which the ⋯ does not. With `trailing` unset the row is byte-identical to the two grammars above — that is the contract that lets ~57 existing call sites ignore this. **This is the tap-reachable path, not a duplicate of the swipe:** `SwipeActionRow` (below) is an accelerator that may never be the only route to an action, and it deliberately excludes destroys that are irreversible for other people — a ⋯ is where those live, behind a `ConfirmDialog` (§14).
...
- **Chat** — **timestamps follow iMessage (ratified 2026-09-13):** a centred serif-italic 13 stamp opens each conversation WINDOW — the first message and any message more than 60 minutes after the previous one (`opensTimeWindow`, chats-tab.tsx) — and carries the clock time (`TODAY · 3:42 PM`, `YESTERDAY · …`, `TUE · …`, `SAT · SEP 5 · …`; `formatTimeSepLabel`). It is NOT a per-day divider and NOT per message. Per-message times are never printed in the flow: each row parks its 11 muted time `TIME_REVEAL_PX` past the right edge (`MessageTimeLabel`) and the **swipe-left reveal** (below) slides the column in. The long-press menu carries the full stamp as a caption — the tap-reachable path (§0.3). Incoming = ivory bubble r18 (4 at the tail) + 26px avatar + name row (name only); outgoing = plum bubble, cream text, right. Composer = "+" ghost (opens a compact `ActionMenu` anchored ABOVE it — photo/file · GIF · poll — iMessage-style, not a sheet; pass `panelClassName` to swap the desktop skin for an `--ivory` r16 tonal card, the only fill that reads on the flat `--cream` chat surface) · ivory pill input · 44px plum send circle. No emoji button — the touch keyboard carries one.
- **Month grid** — ruled form only (see §0.2).
- **Carousel** (`PocketUpNext`) — scroll-snap, -20px bleed, 82% cards, dots (plum active). Manual only.
- **Swipe row actions** (`SwipeActionRow`, `components/central/swipe-actions.tsx`) — drag a list row sideways to uncover actions behind it (iMessage's row gesture). Leading (right-swipe) and trailing (left-swipe) panels; 76px per action, full row height, icon over an 11px serif/600 label. Both tones fill `--pocket-track` (§2's mobile fill token — the family's "behind the card" value, which reads as a layer recessed beneath the `--ivory` row rather than a hole punched through it) and take their weight from TEXT, not fill: `default` = `--body`, `strong` (the weightier action — leave/archive) = `--ink`. Never `--line-2` (border/stroke only, §6) and never `--cream-2` (a desktop token, and lighter than the card it sits behind). One fill across the panel means ADJACENT tiles are separated by a **1px `--line-3` hairline** — the same in-card divider that separates `PocketRow`s (§2), lighter than the fill so it reads as an inset seam — never on the panel's outer edge, where it would read as a border on the card. **Plum is deliberately absent** — it is a surgical accent and a list like Chats already spends it on the unread dot, the scope pill and the create +. Never a danger fill. **Exactly TWO quick actions per row** (ratified 2026-08-24) — Pin one way, and ONE room action the other. Which room action is the only thing that varies, and it is the one that fits that kind of room: Leave where you can leave (a `my` chat), Mute where you cannot (a church chat, a DM). A third tile is not a third option so much as a smaller target for the two people use. Archive and Delete are NOT eligible: both act on the room for every member, and they keep their deliberate path through the surface's own danger zone. **The gesture has two depths.** A short drag uncovers the tile to tap; a drag past the COMMIT distance — half the row, floored at the panel width + 48 — fires that side's `commit` action on release, with no tap. The decision is made on RELEASE, so dragging back before you lift cancels, and a flick never commits — only distance does, because a fast short flick is both how you ask to see the panel and what a mis-swipe looks like. Any room action a full swipe can reach must CONFIRM, on both paths: the swipe carries you to the decision, it does not make it for you. Undo directions (unpin, unmute) don't confirm. **Armed** — past the commit point the flagged tile SWALLOWS the panel: it is the flex child so it absorbs the extra width, any siblings' glyphs fade out, the glyph pops, and the label steps to its committed colour — `--ink` for a `default` tile, `--danger` for a `strong` one, which the contract allows as TEXT and forbids as a fill. The colour is load-bearing whenever the panel holds one tile: there are no siblings to fade and the tile is already the full uncovered width, so the stretch says nothing on its own. Plum stays absent here too. The glyph is pinned to the tile's fixed edge (outer for leading, inner for trailing) so it never drifts to the centre of a slab. The panel paints the tiles' own fill itself — adjacent tiles land on fractional device pixels and the sliver between them would otherwise show the page. Colour and the seam border live in `.swipe-tile` (`app/globals.css`), never inline: an inline declaration outranks every state rule. Inside a `PocketRowCard` pass `bleed={18}` so the panel reaches the card edge instead of stopping short of it; the card clips to its own radius. The gesture is an **ACCELERATOR, never the only path** (§0.3) — every action it exposes must also be reachable by tapping through the UI, or a user who never discovers the swipe (and every screen-reader user) is stranded. Safety properties are inherited from `useEdgeSwipeBack` and its `inHorizontalScroller` guard is imported, not copied (Convention #7): coarse-pointer only, direction-locked, multi-touch guarded, and it **ignores a touch starting within 24px of the left edge** so back-swipe (Convention #22) is never contested. `touch-action: pan-y` on the foreground leaves vertical scrolling and pull-to-refresh entirely to the browser. Open state is CONTROLLED by the parent, which is what guarantees only one row is open at a time and lets Android back close it. A destructive action that is irreversible FOR OTHER PEOPLE does not belong here — put the reversible sibling on the swipe and keep the destroy in the surface's danger zone (why Chats offers Archive, not Delete).
- **Swipe-to-reply on a message bubble** (`useSwipeToReply`, `components/central/use-swipe-to-reply.ts`) — drag a chat bubble RIGHT ~56px and release to load it into the composer's reply strip. **One direction for every message, own bubbles included** (ratified 2026-08-20): iMessage, WhatsApp and Signal are all right-for-both and only Telegram inverts, so mirroring by sender would fight muscle memory for no gain. An own bubble is right-aligned and flush to the trailing inset, so it travels off the screen edge — the transcript scroller carries `overflow-x-hidden` to clip it there, exactly as iMessage does. The reveal is a `CornerUpLeft` glyph in `--muted-text` behind the bubble, fading with drag progress; never plum, which the outgoing bubble already spends. Like `SwipeActionRow` this is an **ACCELERATOR, never the only path** (§0.3) — long-press → Reply is unchanged. Safety properties are inherited from `useEdgeSwipeBack`, whose `inHorizontalScroller` guard it imports rather than copies.
- **Swipe-left to reveal times on the transcript** (`useSwipeRevealTimes`, `components/central/use-swipe-reveal-times.ts`, ratified 2026-09-13) — drag the conversation LEFT and every message's time slides in on the right, one beside each bubble, right-aligned to the trailing inset (iMessage's timestamp gesture); release and the column settles home (240ms house ease-out, instant under reduced motion). Listeners on the transcript scroller, ONE `translateX` on the message column — each row's label is already parked 72px past its right edge, so moving the column IS the reveal; no per-row style write, no CSS variable fan-out. HARD STOP at full reveal — the column never travels past 72px however far the finger goes (a rubber-band was tried and read as "keep going", ratified 2026-09-13). Claims LEFTWARD only, so it never contests swipe-to-reply (rightward) or edge-back (rightward from the left edge); a vertical-dominant move is released to the scroller before anything is claimed. Coarse-pointer only, multi-touch guarded, imports the same `inHorizontalScroller` guard. `onLock` cancels the shared long-press timer (Convention #7) — a leftward drag that begins on a bubble must not open the context menu mid-drag. Desktop has no drag: the same label shows on bubble hover, beside the bubble (Messenger web). Guarded by `e2e/chat-timestamps.mobile.spec.ts`.
