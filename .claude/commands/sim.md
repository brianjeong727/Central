...
7. **Verify + report** in one sentence: which port the sim now points at, which device, and — always — the reminder that `capacitor.config.ts` is carrying an uncommitted dev overlay (revert with `git checkout -- capacitor.config.ts` before any commit that would touch it; the pre-ship check is that `git diff capacitor.config.ts` shows no localhost URL).
