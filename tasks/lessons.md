...
- **A swallowed Supabase error + `revalidate:false` = a permanently poisoned cache.** The chat sidebar intermittently showed "No church chats" until many refreshes. Chain: an RPC fetcher destructured only `{ data }` (not `error`), so a transient failure (session warming, network) returned `data ?? [] === []` instead of throwing → a realtime-triggered `globalMutate(key, asyncFn, { revalidate:false })` wrote that `[]` to the cache with NO recovery fetch → `dedupingInterval:5000` blocked the mount revalidation → `isLoading:false, data:[]` → the empty-state branch (which only checked `isLoading`) rendered "No church chats" and the `data.length`-driven unread badge zeroed. Rules: (1) **a Supabase fetcher used with SWR MUST `if (error) throw error`** — returning `[]` on failure makes a transient blip indistinguishable from genuinely-empty and (with `keepPreviousData`) defeats the very feature that would have hidden it. (2) **An empty-state render must gate on `!isLoading && !error`, never on `data.length===0` alone** — fold errored-with-no-data into the loading branch so a spinner (or stale data) shows, never a false "nothing here". (3) A fire-and-forget `globalMutate(..., {revalidate:false})` whose updater can reject must be `.catch()`'d AND must let the updater throw (SWR keeps previous data on a rejected mutate) so a failed background refetch is a silent no-op, not a cache-poisoning write.
