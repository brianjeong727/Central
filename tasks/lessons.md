...
## Settings changes never apply immediately — stage behind Save (2026-07-11)
Brian has corrected this more than once ("once again, settings changes should always have a confirmation"). The chat pin/mute toggles initially applied on tap (optimistic write per Convention #4) and were reworked to the staged idiom.
**Rule:** on any SETTINGS surface (ministry settings, chat settings, team settings, moderation, automations — anything that configures rather than converses), a control change updates PENDING local state only; the DB write happens on an explicit "Save changes" commit, with a Cancel that reverts pending. The system pattern is settings-tab's `pending*Settings` + edit/cancel/save controls — scale it to the surface, don't invent a new one. Convention #4 (optimistic updates) applies to conversational/user-facing writes (messages, reactions, RSVPs), NOT to settings commits; once Save is pressed, the write may still patch caches optimistically with rollback.
**How to apply:** pending state object + dirty check (`pending !== saved`) → Save/Cancel affordance appears only when dirty → save handler performs the writes and cache patches. Destructive actions additionally keep their ConfirmDialog.

## §Menu / popover viewport collision (2026-07-11)

**Pattern (recurred 3×):** hand-rolled kebab / overflow dropdowns anchored directly below their trigger with `absolute top-8 right-0` (or a `position:fixed` measured only from the trigger's bottom) and **no viewport-collision handling**. Two failure modes: (a) the last card's menu clips off the **viewport bottom** on desktop (reported on the last announcement card's ⋯); (b) menus rendered `absolute` **inside an `overflow-hidden` card** get clipped by the card ancestor (both mobile `AnnouncementCard` variants). Same class of bug appeared in announcements (desktop + 2 mobile variants), settings member-row role menu, and the profile journal kebab (×3).

**Rule:** all action/kebab menus use the shared `components/central/action-menu.tsx` (`ActionMenu`). It `createPortal`s to `document.body` at `position:fixed`, measures its **own** height in `useLayoutEffect` before paint (renders invisible for the first frame), **flips above** the trigger when placing below would clip the viewport bottom, and clamps horizontally into the viewport. Closes on outside-click / Escape / scroll / resize. Never re-introduce an `absolute`-positioned dropdown inside a card, and never anchor a fixed menu off only the trigger's `bottom` without a flip test. API: pass `items: {key,label,icon?,tone?,onSelect}[]` (or `children:(close)=>ReactNode` for menus needing section labels / radios / dividers, e.g. settings "Set role"), plus `renderTrigger` and `align`.

**Reference:** measurement/flip approach mirrors the chat context-menu in `app/home/tabs/message-row.tsx` (~L162–191, `useLayoutEffect` + `placeBelow`) — reused, not shared from there (that file is frozen: chat interaction conventions). `ActionMenu` matches the legacy `DesktopActionMenu` panel styling (`--cream-panel`, `rounded-xl`, danger tone via `--danger`), z-index 200 (the overlay default per DESIGN_SYSTEM §4.17).
