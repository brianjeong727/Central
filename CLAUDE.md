...
15. **Header-right CTAs — object-config only:** The top/object header (`TabPageHeader` right slot) carries only object-level actions — the Settings **gear** (collapse to a kebab at 3+). **Create / add / generate buttons never go there.** Every create is a plum primary in the **body** content header of the collection it fills — use `ContentHeader` (or `SectionHeader`) + `ContentActionButton` (primary), per DESIGN_SYSTEM §3.2 Zone C. On a multi-section workspace the top shows the workspace name + gear, and each section’s create lives in that section’s own body header; the parent still owns the trigger (the `generateTrigger` / `startNewTrigger` / `newSemesterTrigger` counter pattern) but wires it to the body header’s action, not the page header. Canonical example: the six sections in `StudentOrgTeamHome`. (Single-feed exception per §3.2: when a page title directly heads its one collection, the create may sit in the title row’s right slot — that slot *is* the content header.) The `HeaderActionButton`-in-`TabPageHeader` create pattern is retired for multi-section workspaces.
