...
| `components/central/use-swipe-reveal-times.ts` | `useSwipeRevealTimes` + `TIME_REVEAL_PX` — iMessage's timestamp gesture: drag the transcript LEFT and every message's parked time (`MessageTimeLabel` in `message-row.tsx`, absolutely positioned `TIME_REVEAL_PX` past the row's right edge, clipped by the scroller's `overflow-x-hidden`) slides in beside its bubble. Listeners on the scroller (`scrollContainerRef`), one `translateX` on the message column (`messageTrackRef`) — no per-row work. Claims LEFTWARD drags only (swipe-to-reply owns rightward, edge-back owns the left edge); same safety set as the other swipe hooks, `inHorizontalScroller` imported not copied; `onLock` → `handlePointerCancel` so the shared long-press timer dies (Convention #7). Per-message times are NOT printed in the flow anywhere; the centred stamp opens a conversation WINDOW (>60 min gap, `opensTimeWindow` in chats-tab.tsx, label `formatTimeSepLabel` in utils.ts), and the long-press menu carries the full stamp as `MessageMenuOverlay.caption`. Desktop shows the label on bubble hover. Guarded by `e2e/chat-timestamps.mobile.spec.ts`. |
