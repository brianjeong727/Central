...
- **Safe area:** the app shell root owns `env(safe-area-inset-top)` (`home-app.tsx`), so everything mounted inside it is already inset. A **shell-escaping** overlay (`fixed inset-0` — ChatScreen, chat settings, create-chat) escapes that padding and must add it itself, via `POCKET_OVERLAY_PAD_TOP_CLS` (`components/central/pocket.tsx`) = inset + the standard 12px chrome pad. **Never a hardcoded floor.** The old rule here was `pt-12`, which became `max(env(safe-area-inset-top), 48px)` in three hand-typed copies: wherever the inset reports 0 (browser, simulator, notchless device) chat opened 36px lower than every other screen — the one place the app visibly broke its own chrome rhythm (Convention #27).
