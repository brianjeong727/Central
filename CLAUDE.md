...
| `app/home/components/video-stage.tsx` | The video layouts. 1:1 = the other person full-bleed with you as a corner inset; group = an even grid, active speaker ringed, nobody larger than anyone else. Column count comes from the PARTICIPANT COUNT, not a breakpoint — what makes a face too small is how many share the screen, and that is the same number on any device. A muted camera publication renders the monogram, never a frozen last frame. |
