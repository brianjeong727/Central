...
## Database Migrations
Never create migration files in the `supabase/` folder and ask the user to run them manually. The Supabase MCP is *usually* connected — **verify it at task start** with a `ToolSearch` for `mcp__supabase__execute_sql` before planning around it. When it is available, always run migrations directly against the database using the MCP. When it is absent, do NOT improvise a transport (reading the Management API token out of `~/.claude.json` and curling it is blocked, correctly): author the migration as a `.sql` file in `supabase/`, get the `rls-reviewer`'s static design review, and hand Brian the file plus its verification queries — marking the task **migration pending**, since the reviewer's live AFTER-pass probes are still owed. When a schema change is needed, execute it immediately as part of the task. After running, verify the tables and policies were created correctly by querying the database before moving on.
