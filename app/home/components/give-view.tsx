  ...
  return (
    <div style={{ background: "var(--cream)", border: "1px solid var(--line)", borderRadius: 16, padding: "18px 20px" }}>
      ...
      <div style={{ padding: "12px 14px", border: "1px solid var(--line)", background: "var(--cream)", borderRadius: 12, marginBottom: 12 }}>
        <p style={{ fontSize: 13.5, color: "var(--ink)", fontWeight: 400, lineHeight: 1.2 }}>{zelleInfo}</p>
...
function useZelleInfo(ministryId: string) {
  ...
  return { zelleInfo, setZelleInfo, loading }
}
...
export function GiveView({
  ...
  const { zelleInfo, setZelleInfo, loading } = useZelleInfo(ministryId)
              ...
              <div>
                {editing ? (
                  ...
                  <div>
                    ...
                    <button onClick={() => { setEditValue(""); setEditing(true) }} style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "10px 20px", background: "var(--plum-2)", color: "var(--cream)", borderRadius: 10, fontSize: 13, fontWeight: 500, border: "none", cursor: "pointer" }}>
