        ...
        {loading ? <Spinner /> : editing || (!zelleInfo && isAdmin) ? (
          ...
          <div className="md:grid md:gap-5" style={{ gridTemplateColumns: "1.3fr 1fr" }}>
            <div className="border-0 bg-[var(--ivory)] rounded-[var(--r-pocket)] md:border md:border-[var(--line)] md:bg-[var(--cream)] md:rounded-[16px] mb-4 md:mb-0" style={{ padding: "28px 28px 24px" }}>
              ...
              {zelleFallback && <p style={{ fontSize: 13, color: "var(--body)", textAlign: "center", lineHeight: 2, marginBottom: 10 }}>Open Zelle on your phone and send to <strong style={{ color: "var(--ink)" }}>{zelleName ? `${zelleName} (${zelleInfo})` : zelleInfo}</strong></p>}
